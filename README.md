...
This deploys `hyprland.lua`, all `modules/*.lua` (including `host.lua`), and
`hypridle.conf`. The Noctalia-generated files (`noctalia.lua`,
`hyprtoolkit.conf`) are regenerated by Noctalia at runtime, so they are not
shipped in the repo.
...
These files are generated by Noctalia's theme/applying logic and are
not versioned (see `.gitignore`). They appear on disk after the first
Noctalia run and are machine-specific:
